    ...
    for line in file:
        if float(line[:15]) >= timestamp1 and float(line[:15]) <= timestamp2:
            ...
            if value_list[7]in users:
                ...
                # Aqui estoy tomando como errores los codigos 000 tambien
                if int(status[1]) >= 100 and int(status[1]) < 400:
                    try:
                        # Cant de (bytes reciv*8 )/elapsed time
                        throughput = float(
                            value_list[4])*8/float(value_list[1])
...
# print(value_list)
    ...
    print('created users file')
    make_file(users)
    file.close()
    print('Finished')
    return 'El archivo esta listo para su analisis'
